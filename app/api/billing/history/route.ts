...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: error.message || 'An error occurred while getting billing history' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
      ...
      if (invoice.subscription) {
        ...
        if (subscription.items.data.length > 0) {
          ...
          for (const [key, plan] of Object.entries(stripe)) {
            if (typeof plan === 'object' && plan !== null && 'priceId' in plan && plan.priceId === priceId) {
              planId = plan.id;
              planName = plan.name;
              break;
            }
    ...
    return NextResponse.json(
      { error: error.message || 'An error occurred while syncing billing history' },
      { status: 500 }
    );
  }
}
